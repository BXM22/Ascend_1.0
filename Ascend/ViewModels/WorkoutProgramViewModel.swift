...
class WorkoutProgramViewModel: ObservableObject {
    ...
    /// Fills each non-rest day with a generated template and copies exercises onto the program day (detail view + start workout).
    func populateProgramDaysWithAutoGeneratedWorkouts(programId: UUID, settings: WorkoutGenerationSettings, templatesViewModel: TemplatesViewModel) {
        guard let programIndex = programs.firstIndex(where: { $0.id == programId }) else { return }
        guard !programs[programIndex].days.isEmpty else { return }
        
        for dayIndex in 0..<programs[programIndex].days.count {
            if programs[programIndex].days[dayIndex].isRestDay { continue }
            guard let result = ensureTemplateForDay(dayIndex: dayIndex, inProgram: programId, settings: settings, templatesViewModel: templatesViewModel) else { continue }
            let templateExercises = result.template.exercises.map { te in
                ProgramExercise(
                    name: te.name,
                    sets: te.sets,
                    reps: te.reps,
                    ...
                    targetHoldDuration: te.targetHoldDuration
                )
            }
            programs[programIndex].days[dayIndex].exercises = templateExercises
            programs[programIndex].days[dayIndex].estimatedDuration = result.template.estimatedDuration
        }
    }
